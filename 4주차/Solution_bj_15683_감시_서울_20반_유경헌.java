...
public class Solution_bj_15683_감시_서울_20반_유경헌 {
	...
	static void go(int x, int y, int d) { // 감시 구역 체크
		if (!cameraCheck[x][y])
			office[x][y]--;
		int nx = x + dx[d];
		int ny = y + dy[d];
	...
	static void back(int x, int y, int d) { // 감시 구역 해제 
		if (!cameraCheck[x][y])
			office[x][y]++;
		int nx = x + dx[d];
		int ny = y + dy[d];

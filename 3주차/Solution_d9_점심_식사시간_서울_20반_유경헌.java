...
public class Solution_d9_점심_식사시간_서울_20반_유경헌 {
	static int N, ans, where[], map[][], times[][];
	static boolean[] v;
	static ArrayList<Info> points;
	static Queue<Info> stairs1, stairs2;
	static Info[] stairsPoint;
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			for (int i = 0; i < N; i++) {
				...
				for (int j = 0; j < N; j++) {
					...
					if (map[i][j] == 1)
						points.add(new Info(i, j));
					else if (map[i][j] != 0) {
						stairsPoint[sIdx++] = new Info(i, j, map[i][j]);
					}
				}
			}
	...
	static int calcTime(Info p1, Info p2) {
	...
	static void dfs(int idx) {
		if (idx == points.size()) {
			...
			int checkCnt = 0, time = 0;
			while (checkCnt != points.size()) {
				for (Info p : stairs1) {
					p.t++;
				}
				for (Info p : stairs2) {
				...
				for (int i = 0; i < points.size(); i++) {
					...
					if (p.t <= 0 && !v[i]) {
						if (where[i] == 0 && stairs1.size() < 3) {
							stairs1.offer(new Info(0, 0, 0));
							v[i] = true;
						} else if (where[i] == 1 && stairs2.size() < 3) {
							stairs2.offer(new Info(0, 0, 0));
							v[i] = true;
						}
					}
				}
